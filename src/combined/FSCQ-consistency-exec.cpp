...
void mount_crashed(char* crashed_img, char *crashed_mpoint) {
    ...
    if (!child_pid) {
        ...
        args[4] = const_cast<char*>("big_writes,atomic_o_trunc,use_ino");
        args[5] = const_cast<char*>(crashed_mpoint);
        args[6] = NULL;
        execvp(args[0], args);
    }
...
int main(int argc, char *argv[]) {
    ...
    if (argc < 5) {
        printf("Usage: ./fscq-cc mnt_point prog_path emulator_path img_path\n");
        printf("** Make sure the arguments are aligned with the AFL instance **\n");
        printf("e.g.,\n");
        printf("AFL: AFL_NO_FORKSRV=1 AFL_FAST_CAL=1\n");
        printf("     ./combined/afl-fscq-syscall/afl-fuzz -t 1000000 -m none\n");
        printf("     -f /tmp/prog-1 -i ./seed-fscq -o output -k\n");
        printf("     -e /tmp/mosbench/tmpfs-separate/0/disk.img\n");
        printf("     -p /tmp/mosbench/tmpfs-separate/0/tmp-1.img -u 1\n");
        printf("     -- ./fscq/src/fscq /tmp/mosbench/tmpfs-separate/0/tmp-1.img\n");
        printf("     -f -o big_writes,atomic_o_trunc,use_ino /tmp/mnt-1\n");
        printf("EXE: ./fscq-cc /tmp/mnt-1 /tmp/prog-1 ./emulator/fscq.py /tmp/tmp-1.img\n");
        return 1;
    }
